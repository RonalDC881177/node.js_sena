  ...
  try {
    ...
    if (!name || typeof name !== "string" || !name.trim()) {
      return res.status(400).json({
        success: false,
        message:
          "El nombre de la categoria es obligatorio y debe ser texto valido",
      });
    }
...
exports.getCategories = async (req, res) => {
  try {
    ...
    const categories = await Category.find(activeFilter).sort({
      createdAt: -1,
    });
    res.status(200).json({
      success: true,
...
exports.deleteCategory = async (req, res) => {
  try {
    ...
    if (isHardDelete) {
      ...
      const subIds = (await Subcategory.find({ category: req.params.id })).map(
        (s) => s._id,
      );
      ...
      res.status(200).json({
        success: true,
        message:
          "Categoria eliminada permanentemente y sus subcategorias y productos relacionados",
        data: {
          category: category,
        },
      });
      ...
      res.status(200).json({
        success: true,
        message:
          "Categoria desactivada exitosamente como sus subcategorias y productos relacionados",
        data: {
          category: category,
