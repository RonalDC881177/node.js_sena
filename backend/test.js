...
async function request(method, endpoint, body = null, headers = {}) {
  // Función reutilizable para hacer peticiones HTTP al API
  const options = {
    // Objeto de opciones que se pasa a fetch()
  ...
  try {
    // Intenta hacer la petición HTTP; si falla (red caída, servidor apagado) lo captura el catch
    ...
    // Captura errores de red o si el servidor no está corriendo
...
function log(test, status, details = "") {
  // Imprime el resultado de un test con ícono verde/rojo y nombre del test
...
function error(endpoint, response) {
  // Imprime detalles del error si el test falló (mensaje y detalle técnico)
  if (response.data?.message)
    console.log(`   \u2514\u2500 Error: ${response.data.message}`); // Imprime el mensaje de error del API si existe
  if (response.data?.error)
    console.log(
      `   \u2514\u2500 Detalle: ${JSON.stringify(response.data.error)}`,
    ); // Imprime el detalle técnico del error si existe
}

// Suite de pruebas
async function runTests() {
  // Función principal que ejecuta todos los tests en orden
  ...
  // Login fallido
  res = await request("POST", "/auth/signin", {
    // Envía petición de login con contraseña incorrecta
    //username: 'admin',
    email: "admin@example.com",
    password: "wrongpassword", // Contraseña incorrecta para probar que el API la rechaza
  });
  log(
    "Login fallido rechazado",
    !res.ok && res.status === 401,
    `(Status: ${res.status})`,
  ); // Verifica que el API retornó 401
  ...
  log(
    "GET /users",
    usersOk,
    `(${Array.isArray(usersArray) ? usersArray.length : "?"} usuarios)`,
  ); // Imprime resultado con cantidad de usuarios
  ...
  if (userId) {
    // Solo ejecuta si el usuario fue creado exitosamente
  ...
  if (userId) {
    // Solo ejecuta si el usuario fue creado exitosamente
    res = await request("PUT", `/users/${userId}`, {
      email: "updated@example.com",
    }); // Petición PUT para cambiar el email
    log(
      "PUT /users/:id (actualizar)",
      res.ok && res.status === 200,
      `(Status: ${res.status})`,
    ); // Verifica que se recibió 200 OK
  }
  ...
  log(
    "GET /categories",
    catsOk,
    `(${Array.isArray(catsArray) ? catsArray.length : "?"} categorías)`,
  ); // Imprime resultado con cantidad
  if (!catsOk) error("/categories", res); // Si falló, imprime detalles del error

  // Crear categoría
  const newCategory = {
    // Datos de la categoría de prueba con timestamp para evitar duplicados
  ...
  if (categoryId) {
    // Solo ejecuta si la categoría fue creada exitosamente
  ...
  if (categoryId) {
    // Solo ejecuta si la categoría fue creada exitosamente
    res = await request("PUT", `/categories/${categoryId}`, {
      name: "Updated Category",
    }); // Petición PUT para cambiar el nombre
    log(
  ...
  log(
    "GET /subcategories",
    subcatsOk,
    `(${Array.isArray(subcatsArray) ? subcatsArray.length : "?"} subcategorías)`,
  ); // Imprime resultado
  if (!subcatsOk) error("/subcategories", res); // Si falló, imprime detalles del error

  // Crear subcategoría
  const newSubcategory = {
    // Datos de la subcategoría de prueba con timestamp para evitar duplicados
  ...
  if (subcategoryId) {
    // Solo ejecuta si la subcategoría fue creada exitosamente
    ...
    log(
      ...
      subcatFound,
      `(ID: ${subcategoryId.substring(0, 8)}...)`,
    ); // Imprime resultado
  ...
  if (subcategoryId) {
    // Solo ejecuta si la subcategoría fue creada exitosamente
    res = await request("PUT", `/subcategories/${subcategoryId}`, {
      name: "Updated Subcategory",
    }); // Petición PUT para cambiar nombre
    log(
      "PUT /subcategories/:id (actualizar)",
      res.ok && res.status === 200,
      `(Status: ${res.status})`,
    ); // Verifica 200 OK
  }
  ...
  log(
    "GET /products",
    prodsOk,
    `(${Array.isArray(prodsArray) ? prodsArray.length : "?"} productos)`,
  ); // Imprime resultado
  if (!prodsOk) error("/products", res); // Si falló, imprime detalles del error

  // Crear producto
  const newProduct = {
    // Datos del producto de prueba con timestamp para evitar duplicados
  ...
  if (productId) {
    // Solo ejecuta si el producto fue creado exitosamente
  ...
  if (productId) {
    // Solo ejecuta si el producto fue creado exitosamente
    res = await request("PUT", `/products/${productId}`, {
      price: 89.99,
      stock: 8,
    }); // Petición PUT para cambiar precio y stock
    log(
  ...
  log(
    "GET /statistics",
    res.ok && res.status === 200 && res.data?.totalProducts !== undefined,
    `(Status: ${res.status})`,
  ); // Verifica 200 y que exista el campo totalProducts
  if (res.ok) {
    // Si la petición fue exitosa
    console.log(
      `   Productos: ${res.data.totalProducts}, Categorías: ${res.data.totalCategories}, Usuarios: ${res.data.totalUsers}`,
    ); // Imprime los totales principales
  }
  ...
  // Desactivar producto
  if (productId) {
    // Solo ejecuta si el producto existe
    ...
    if (res.ok) {
      ...
      console.log(
        `   \u2514\u2500 Producto desactivado (active=${prodData?.active})`,
      ); // Imprime el nuevo valor del campo active
    }
  }

  // Desactivar subcategoría (cascada a productos)
  if (subcategoryId) {
    // Solo ejecuta si la subcategoría existe
    ...
    log(
      "DELETE /subcategories/:id (cascada)",
      subDeleteOk,
      `(Status: ${res.status})`,
    ...
    if (res.ok) {
      ...
      if (subData?.productsDeactivated !== undefined) {
        // Si la respuesta incluye conteo de productos afectados
        console.log(
          `   \u2514\u2500 Subcategoría desactivada, productos desactivados: ${subData.productsDeactivated}`,
        ); // Muestra cuantos productos se desactivaron en cascada
      } else {
        // Si no incluye el conteo, muestra el estado de la subcategoría
        console.log(
          `   \u2514\u2500 Subcategoría desactivada (active=${subData?.subcategory?.active})`,
        );
      }
    }
  }
  ...
  if (categoryId) {
    // Solo ejecuta si la categoría existe
    ...
    if (res.ok) {
      ...
      console.log(
        `   \u2514\u2500 Subcategorías afectadas: ${catData?.subcategoriesDeactivated || 0}`,
      ); // Muestra cuantas subcategorías se desactivaron
      console.log(
        `   \u2514\u2500 Productos afectados: ${catData?.productsDeactivated || 0}`,
      ); // Muestra cuantos productos se desactivaron
    }
  }

  // Desactivar usuario
  if (userId) {
    // Solo ejecuta si el usuario existe
    ...
    if (res.ok) {
      ...
      console.log(
        `   \u2514\u2500 Usuario desactivado (active=${userData?.active})`,
      ); // Imprime el nuevo valor del campo active
    }
  }
...
// Ejecutar tests
runTests().catch((err) => {
  // Ejecuta la suite de tests; si lanza error no capturado lo maneja el callback
